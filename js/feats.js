"use strict";class FeatsPage extends ListPage{constructor(){const a=getSourceFilter(),b=getAsiFilter(),c=new Filter({header:"Prerequisite",items:["Ability","Race","Proficiency","Spellcasting"]});super({dataSource:"data/feats.json",filters:[a,b,c],filterSource:a,listValueNames:["name","source","ability","prerequisite","uniqueid"],listClass:"feats",sublistValueNames:["name","ability","prerequisite","id"],sublistClass:"subfeats",dataProps:["feat"]}),this._sourceFilter=a}getListItem(a,b){const c=a.name,d=Renderer.getAbilityData(a.ability);d.asText||(d.asText=STR_NONE),a._fAbility=d.asCollection.filter(b=>!d.areNegative.includes(b));let e=Renderer.feat.getPrerequisiteText(a.prerequisite,!0);e||(e=STR_NONE);const f=new Set;return(a.prerequisite||[]).forEach(a=>f.add(...Object.keys(a))),a._fPrereq=[...f].map(a=>a.uppercaseFirst()),a._slAbility=d.asText,a._slPrereq=e,this._sourceFilter.addItem(a.source),`
		...
		`}doLoadHash(a){const b=this._dataList[a];$("#pagecontent").empty().append(RenderFeats.$getRenderedFeat(b)),ListUtil.updateSelected()}doLoadSubHash(a){a=this._filterBox.setFromSubHashes(a),ListUtil.setFromSubHashes(a)}}const featsPage=new FeatsPage;window.addEventListener("load",()=>featsPage.pOnLoad());