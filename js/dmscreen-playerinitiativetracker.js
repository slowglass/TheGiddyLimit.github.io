"use strict";class InitiativeTrackerPlayer{static make$tracker(a,b){const c=$(`<div class="initp__meta"/>`).hide(),d=$(`<div class="initp__header"/>`).hide(),e=$(`<div class="initp__rows"/>`).hide(),f=$$`<div class="initp__wrp_active">
			...
			${e}
				...
				</div>`.appendTo(g.$wrpInitial),j=new InitiativeTrackerPlayerUi(g,a,b,c,d);j.init()}),i=$(`<button class="btn btn-primary" style="min-width: 200px;">Connect to Local Tracker</button>`).click(()=>{const b=a.getPanelsByType(PANEL_TYP_INITIATIVE_TRACKER).map(a=>a.tabDatas.filter(a=>a.type===PANEL_TYP_INITIATIVE_TRACKER).map(a=>a.$content.find(`.dm__data-anchor`))).flat();if(!b.length)JqueryUtil.doToast({content:"No local trackers detected!",type:"warning"});else if(1===b.length)b[0].data("doConnectLocal")(g);else{h.detach(),i.detach();const a=$(`<select class="form-control input-xs mr-1">
							<option value="-1" disabled>Select a local tracker</option>
						</select>`).change(()=>a.removeClass("error-background"));b.forEach((b,c)=>a.append(`<option value="${c}">${b.data("getSummary")()}</option>`)),a.val("-1");const c=$(`<button class="btn btn-primary btn-xs">OK</button>`).click(async()=>"-1"===a.val()?a.addClass("error-background"):void(await b[+a.val()].data("doConnectLocal")(g),e.remove(),d.remove(),g.$wrpInitial.append(h).append(i))),d=$$`<div class="flex-vh-center mb-2">
							${a}
							${c}
						</div>`.appendTo(g.$wrpInitial),e=$(`<button class="btn btn-default btn-xs">Back</button>`).click(()=>{e.remove(),d.remove(),g.$wrpInitial.append(h).append(i)}).appendTo(g.$wrpInitial)}});return g.$wrpInitial=$$`<div class="flex-vh-center h-100 flex-col">
			${h}
			${i}
		</div>`.appendTo(f),f}}class InitiativeTrackerPlayerMessageHandlerScreen extends InitiativeTrackerPlayerMessageHandler{constructor(){super(!0),this._$wrpInitial=null}initUi(){this._isUiInit||(this._isUiInit=!0,this._$meta.show(),this._$head.show(),this._$rows.show(),this._$wrpInitial.hide(),$(window).on("beforeunload",a=>{if(this._clientData.client.isActive){return(a||window.event).message="The connection will be closed","The connection will be closed"}}))}set $wrpInitial(a){this._$wrpInitial=a}get $wrpInitial(){return this._$wrpInitial}}