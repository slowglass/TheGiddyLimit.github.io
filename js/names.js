"use strict";const JSON_URL="data/names.json";let nameList;const renderer=Renderer.get();function makeContentsBlock(a,b){let c="<ul>";return b.tables.forEach((d,e)=>{const f=getTableName(b,d);c+=`<li>
			...
			</li>`}b.append(c),list=ListUtil.search({valueNames:["name"],listClass:"names"}),Hist.init(!0)}function showHideList(a){const b=$(a);b.next(`ul`).toggle()}function loadHash(a){renderer.setFirstSection(!0);const[b,c]=a.split(",").map(a=>+a),d=nameList[b],e=d.tables[c].table,f=getTableName(d,d.tables[c]),g=d.tables[c].diceType;let h=`
		<tr>
			<td colspan="6">
